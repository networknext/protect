                    ...
                    if ( ip->daddr == config->public_address && udp->dest == config->port )
                    {
                        ...
                        switch ( packet_data[0] )
                        {
                            case NEXT_CLIENT_BACKEND_PACKET_INIT_REQUEST:
                            {
                                ...
                                int result = bpf_next_secretbox_encrypt( (__u8*) &response->backend_token, sizeof(struct next_client_backend_token_t), client_backend_private_key, NEXT_SECRETBOX_KEY_BYTES );
                                if ( result != 0 )
                                {
