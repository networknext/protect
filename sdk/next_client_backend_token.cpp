...
static const char * context = "client backend token";
...
int next_write_client_backend_token( next_client_backend_token_t * token, uint8_t * output, const uint8_t * private_key )
{
    ...
    const int encrypt_result = hydro_secretbox_encrypt( output, (uint8_t*) token + sizeof(token->crypto_header), sizeof(next_client_backend_token_t) - sizeof(token->crypto_header), 0, context, private_key );
    if ( encrypt_result == 0 )
